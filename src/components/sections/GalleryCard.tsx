...
export function GalleryCard({ item, onOpen, className }: GalleryCardProps) {
  return (
    <Card className={cn("h-full p-24", className)}>
      <BeforeAfterSlider
      ...
      <div className="mt-24 flex items-center justify-between gap-16">
        ...
        {onOpen ? (
          <button
            type="button"
            onClick={() => onOpen(item)}
            className="inline-flex h-48 w-48 shrink-0 items-center justify-center rounded-button text-primary transition duration-DEFAULT hover:bg-gray-light"
